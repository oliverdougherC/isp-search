...
| Bounded retries with backoff, by failure class  | queue `retryLimit`, `retryDelay`, `retryBackoff`, `retryDelayMax`; `retryLimitForOutcome` maps `classifyRetry` to budgets (explicit results and user actions never retry) | "retries a failed job with backoff and dead-letters it" + unit test |
